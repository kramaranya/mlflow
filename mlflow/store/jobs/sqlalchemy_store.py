...
class SqlAlchemyJobStore(AbstractJobStore):
    ...
    def report_job_result(
        ...
        if status == JobStatus.SUCCEEDED:
            if result is None or error_message is not None:
                raise MlflowException.invalid_parameter_value(
                    "Succeeded job results must set `result` and leave `error_message` unset."
                )
            payload = result
    ...
    def _get_sql_job(
        self, session, job_id, *, populate_existing: bool = False
    ) -> SqlJob:
