...
def test_retry_or_fail_job_rechecks_retry_budget_before_updating(monkeypatch, tmp_path: Path):
    ...
    original_conditional_status_update = store._conditional_status_update
    injected_budget_race = False
    ...
    def conditional_status_update(
        ...
        nonlocal injected_budget_race

        is_retry_update = (
            not injected_budget_race
            and target_job_id == job.job_id
            and current_statuses == (JobStatus.RUNNING,)
            and values.get(SqlJob.status) == JobStatus.PENDING.to_int()
        )
        if is_retry_update:
            injected_budget_race = True
            with store.ManagedSessionMaker() as competing_session:
                competing_session.query(SqlJob).filter(SqlJob.id == job.job_id).update({
                    SqlJob.retry_count: 3,
                })

        return original_conditional_status_update(
            session,
            target_job_id,
            current_statuses,
            values,
    ...
    monkeypatch.setattr(store, "_conditional_status_update", conditional_status_update)
